...
Application française qui transforme neuf réponses en trois pistes de SaaS, un prompt de construction et un plan sur 30 jours, vendus en abonnement. Next.js 15 App Router, TypeScript, Tailwind CSS, Supabase, Whop et Resend. Aucun dossier n’est écrit par une IA : les textes sont rédigés à l’avance pour chaque idée et assemblés selon les réponses.
...
4. Le webhook confirme le paiement, publie le dossier et les bonus de la formule en quelques secondes, puis envoie le lien par email. La page `/dossier/<lien>` relance la publication si nécessaire. La base de données refuse toute publication avant la confirmation du paiement.
...
- Abonnements Whop : webhook signé et idempotent, accès lié à l’abonnement, résiliation en fin de période depuis le dossier, remboursement du premier paiement sous 48 heures, rappel par email avant le renouvellement des formules 3 et 12 mois.
- Dossier accessible par lien personnel : trois idées, copie et export `.md` du prompt, plan à cocher, vidéos et plan de A à Z selon la formule.
- Resend après confirmation de paiement, journal d’envoi et reprise des erreurs.
- Cinq migrations PostgreSQL : RLS sur toutes les tables, aucun droit pour les rôles du navigateur, transactions réservées au serveur.
...
La troisième migration retire tous les droits des rôles `anon` et `authenticated`, ajoute le lien d’accès et l’email de l’acheteur, et impose la publication après paiement. La quatrième passe aux abonnements : paiements et journal d’événements indépendants du prestataire, formule et état de l’abonnement sur le dossier, tables des vidéos, du plan de A à Z et des rappels de renouvellement. La cinquième supprime les lots quotidiens de vidéos et accepte 60 idées pour la formule 12 mois. Les tables de paiement utilisent des clés `ON DELETE RESTRICT` afin de ne pas effacer l’historique de paiement par cascade.
...
3. Créer un webhook vers `https://<domaine>/api/webhooks/whop` avec les événements `payment.succeeded`, `membership.activated`, `membership.deactivated`, `membership.cancel_at_period_end_changed`, `refund.created` et `refund.updated`, puis copier son secret `ws_…` dans `WHOP_WEBHOOK_SECRET`.
4. Whop exige une adresse de retour HTTPS : le paiement se teste sur le déploiement Vercel, pas sur `http://localhost`.
5. Pour le bac à sable, utiliser une clé du bac à sable avec `WHOP_API_URL=https://sandbox-api.whop.com/api/v1`, relancer le script, puis payer avec la carte de test `4242 4242 4242 4242`.
...
Sans `WHOP_API_URL`, le client vise la production : le passage en caisse exige aussi `LEGAL_COMPANY_NAME`, `LEGAL_COMPANY_ADDRESS`, `LEGAL_COMPANY_REGISTRATION` et `NEXT_PUBLIC_CONTACT_EMAIL`. Ce contrôle de présence ne remplace pas la vérification des textes applicables à l’activité.
...
Les emails de dossier et de remboursement sont envoyés après la réponse au webhook. Un échec email n’annule pas le paiement. La tâche `GET /api/internal/retry-emails`, appelée avec l’en-tête `Authorization: Bearer <CRON_SECRET>`, reprend les envois en échec et programme les rappels de renouvellement : pour les formules 3 et 12 mois, un email part entre 35 et 50 jours avant l’échéance si l’abonnement n’est pas résilié. La configuration Vercel fournie l’exécute chaque jour. Les clés d’idempotence Resend ont une fenêtre de 24 heures : voir les limites détaillées dans `docs/backend.md`.
...
4. Appliquer les migrations à la base de cet environnement, puis configurer le webhook Whop et Resend avec les URLs finales.
5. Créer une valeur aléatoire longue pour `CRON_SECRET` et vérifier l’accès à la tâche quotidienne.
6. Faire le parcours complet dans le bac à sable Whop : paiement, dossier et bonus, email, tâche cochée, résiliation et remboursement. Passer en production seulement après cette vérification et la finalisation des informations commerciales.
