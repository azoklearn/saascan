...
describe("Migrations PostgreSQL et droits RLS", () => {
  it("applique les migrations puis vérifie l’accès serveur, l’abonnement, les bonus et le remboursement", async () => {
    ...
    try {
      ...
      `);
      for (const file of ["202609120001_initial_schema.sql", "202609120002_server_transactions.sql", "202609130001_parcours_sans_compte.sql", "202609130002_abonnements_whop.sql", "202609130003_contenus_rediges.sql", "202609130004_sans_emails.sql"]) {
      ...
      expect(tables.rows[0].count).toBe(10);
